...
A **NIfTI phantom** is a single JSON file that describes a numerical MRI
simulation phantom. It defines a set of *tissues*, each carrying physical MR
properties (T1, T2, …). A property is either a single number (spatially
uniform) or a reference to a sub-volume of a **NIfTI** file stored next to the
JSON. Files are validated against
